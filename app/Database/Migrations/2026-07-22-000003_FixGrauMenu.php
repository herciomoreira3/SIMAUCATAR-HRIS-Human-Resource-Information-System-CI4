...
class FixGrauMenu extends Migration
{
    public function up()
    {
        ...
        // Try to find DADUS MESTRE first, fallback to MASTER DATA
        $masterCategory = $this->db->table('user_menu_category')
            ->where('menu_category', 'DADUS MESTRE')
            ->orWhere('menu_category', 'MASTER DATA')
            ->get()
            ->getRowArray();
