...
<div class="row mb-4">
    <div class="col-12">
        <h1 class="h3 mb-0"><strong>Jestaun</strong> Diresaun</h1>
        <p class="text-muted mb-0">Jere no organiza lista diresaun iha sistema.</p>
    </div>
</div>
    ...
    <div class="col-lg-4">
        <div class="card mb-4">
            <div class="card-header">
                <h5 class="card-title">Aumenta Diresaun</h5>
            </div>
            <div class="card-body">
                <form action="<?= base_url('administrador/diresaun') ?>" method="post">
                    <?= csrf_field() ?>
                    <div class="mb-4">
                        <label class="form-label">Naran Diresaun</label>
                        <input type="text" name="naran_diresaun" class="form-control" placeholder="Eskrebe naran diresaun..." required>
                    </div>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title">Lista Diresaun</h5>
                <span class="badge bg-primary-light text-primary"><?= count($diresaun) ?> Total</span>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            <?php foreach($diresaun as $i => $d): ?>
                            <tr>
                                <td><span class="text-muted"><?= $i+1 ?></span></td>
                                <td class="font-medium text-dark"><?= esc($d['naran_diresaun']) ?></td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#modalEdit<?= $d['id'] ?>">
                                            <i data-feather="edit-2" style="width: 14px; height: 14px;"></i>
                                        </button>
                                        <form action="<?= base_url('administrador/diresaun/delete/'.$d['id']) ?>" method="post" class="d-inline" onsubmit="return confirm('Hamos diresaun ne\'e?')">
                                            <?= csrf_field() ?>
                            ...
                            <div class="modal fade" id="modalEdit<?= $d['id'] ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content shadow-lg">
                                        <form action="<?= base_url('administrador/diresaun/update/'.$d['id']) ?>" method="post">
                    <?= csrf_field() ?>
                                            <div class="modal-header bg-light">
                                                <h5 class="modal-title font-medium">Edit Diresaun</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Taka"></button>
                                            </div>
                                            <div class="modal-body p-4">
                                                <div class="mb-0">
                                                    <label class="form-label">Naran Diresaun</label>
                                                    <input type="text" name="naran_diresaun" class="form-control" value="<?= esc($d['naran_diresaun']) ?>" required>
                                                </div>
                                            </div>
                            ...
                            <?php endforeach; ?>
                            <?php if(empty($diresaun)): ?>
                                <tr><td colspan="3" class="text-center py-4 text-muted">La iha dadus diresaun.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
